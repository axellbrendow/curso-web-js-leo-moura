...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            ...
            existsOrError(user.confirmPassword, 'Confirmation password not informed');
            equalsOrError(
                user.password, user.confirmPassword, 'Passwords does not match');

            const userFromDB = await app.db('users')
                .where({ email: user.email }).first();

            if (!user.id) // If id is not set, this is a register request
